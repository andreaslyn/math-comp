...
Monomorphic Variable eqT : rel T.
Monomorphic Variables (zeroT : T) (oppT : T -> T) (addT : T -> T -> T).

Monomorphic Record zmod_quot_mixin_of (Q : Type) (qc : quot_class_of T Q)
...
Monomorphic Record zmod_quot_class_of (Q : Type) : Type := ZmodQuotClass {
...
Monomorphic Structure zmodQuotType : Type := ZmodQuotTypePack {
  zmod_quot_sort :> Type;
...
Monomorphic Definition zmod_quot_class zqT : zmod_quot_class_of zqT :=
  let: ZmodQuotTypePack _ cT as qT' := zqT return zmod_quot_class_of qT' in cT.

Monomorphic Definition zmod_eq_quot_class zqT (zqc : zmod_quot_class_of zqT) :
  eq_quot_class_of eqT zqT := EqQuotClass zqc.

Monomorphic Canonical zmodQuotType_eqType zqT := Equality.Pack (zmod_quot_class zqT).
Monomorphic Canonical zmodQuotType_choiceType zqT :=
  Choice.Pack (zmod_quot_class zqT).
Monomorphic Canonical zmodQuotType_zmodType zqT :=
  GRing.Zmodule.Pack (zmod_quot_class zqT).
Monomorphic Canonical zmodQuotType_quotType zqT := QuotTypePack (zmod_quot_class zqT).
Monomorphic Canonical zmodQuotType_eqQuotType zqT := EqQuotTypePack
...
Monomorphic Definition ZmodQuotType_pack Q :=
...
Monomorphic Definition ZmodQuotMixin_pack Q :=
...
Monomorphic Definition ZmodQuotType_clone (Q : Type) qT cT
  of phant_id (zmod_quot_class qT) cT := @ZmodQuotTypePack Q cT.

Monomorphic Lemma zmod_quot_mixinP zqT :
  zmod_quot_mixin_of (zmod_quot_class zqT) (zmod_quot_class zqT).
Proof. by case: zqT => [] ? [] ? ? []. Qed.

Monomorphic Lemma pi_zeror zqT : \pi_zqT zeroT = 0.
Proof. by case: zqT => [] ? [] ? ? []. Qed.

Monomorphic Lemma pi_oppr zqT : {morph \pi_zqT : x / oppT x >-> - x}.
Proof. by case: zqT => [] ? [] ? ? []. Qed.

Monomorphic Lemma pi_addr zqT : {morph \pi_zqT : x y / addT x y >-> x + y}.
Proof. by case: zqT => [] ? [] ? ? []. Qed.

Monomorphic Canonical pi_zero_quot_morph zqT := PiMorph (pi_zeror zqT).
Monomorphic Canonical pi_opp_quot_morph zqT := PiMorph1 (pi_oppr zqT).
Monomorphic Canonical pi_add_quot_morph zqT := PiMorph2 (pi_addr zqT).

End ZmodQuot.

Notation ZmodQuotType z u a Q m :=
  (@ZmodQuotType_pack _ _ z u a Q _ _ _ _ id id m).
Notation "[ 'zmodQuotType' z , u & a 'of' Q ]" :=
  (@ZmodQuotType_clone _ _ z u a Q _ _ id)
  (at level 0, format "[ 'zmodQuotType'  z ,  u  &  a  'of'  Q ]") : form_scope.
Notation ZmodQuotMixin Q m0 mN mD :=
  (@ZmodQuotMixin_pack _ _ _ _ _ Q _ _ id _ _ id (pi_eq_quot _) m0 mN mD).

Section PiAdditive.

Monomorphic Variables (V : zmodType) (equivV : rel V) (zeroV : V).
Monomorphic Variable Q : @zmodQuotType V equivV zeroV -%R +%R.

Monomorphic Lemma pi_is_additive : additive \pi_Q.
Proof. by move=> x y /=; rewrite !piE. Qed.

Monomorphic Canonical pi_additive := Additive pi_is_additive.
...
Monomorphic Variable eqT : rel T.
Monomorphic Variables (zeroT : T) (oppT : T -> T) (addT : T -> T -> T).
Monomorphic Variables (oneT : T) (mulT : T -> T -> T).

Monomorphic Record ring_quot_mixin_of (Q : Type) (qc : quot_class_of T Q)
...
Monomorphic Record ring_quot_class_of (Q : Type) : Type := RingQuotClass {
...
Monomorphic Structure ringQuotType : Type := RingQuotTypePack {
  ring_quot_sort :> Type;
...
Monomorphic Definition ring_quot_class rqT : ring_quot_class_of rqT :=
  let: RingQuotTypePack _ cT as qT' := rqT return ring_quot_class_of qT' in cT.

Monomorphic Definition ring_zmod_quot_class rqT (rqc : ring_quot_class_of rqT) :
  zmod_quot_class_of eqT zeroT oppT addT rqT := ZmodQuotClass rqc.
Monomorphic Definition ring_eq_quot_class rqT (rqc : ring_quot_class_of rqT) :
  eq_quot_class_of eqT rqT := EqQuotClass rqc.

Monomorphic Canonical ringQuotType_eqType rqT := Equality.Pack (ring_quot_class rqT).
Monomorphic Canonical ringQuotType_choiceType rqT := Choice.Pack (ring_quot_class rqT).
Monomorphic Canonical ringQuotType_zmodType rqT :=
  GRing.Zmodule.Pack (ring_quot_class rqT).
Monomorphic Canonical ringQuotType_ringType rqT :=
  GRing.Ring.Pack (ring_quot_class rqT).
Monomorphic Canonical ringQuotType_quotType rqT := QuotTypePack (ring_quot_class rqT).
Monomorphic Canonical ringQuotType_eqQuotType rqT :=
  EqQuotTypePack (ring_eq_quot_class (ring_quot_class rqT)).
Monomorphic Canonical ringQuotType_zmodQuotType rqT :=
...
Monomorphic Definition RingQuotType_pack Q :=
...
Monomorphic Definition RingQuotMixin_pack Q :=
...
Monomorphic Definition RingQuotType_clone (Q : Type) qT cT
  of phant_id (ring_quot_class qT) cT := @RingQuotTypePack Q cT.

Monomorphic Lemma ring_quot_mixinP rqT :
  ring_quot_mixin_of (ring_quot_class rqT) (ring_quot_class rqT).
Proof. by case: rqT => [] ? [] ? ? []. Qed.

Monomorphic Lemma pi_oner rqT : \pi_rqT oneT = 1.
Proof. by case: rqT => [] ? [] ? ? []. Qed.

Monomorphic Lemma pi_mulr rqT : {morph \pi_rqT : x y / mulT x y >-> x * y}.
Proof. by case: rqT => [] ? [] ? ? []. Qed.

Monomorphic Canonical pi_one_quot_morph rqT := PiMorph (pi_oner rqT).
Monomorphic Canonical pi_mul_quot_morph rqT := PiMorph2 (pi_mulr rqT).

End RingQuot.

Notation RingQuotType u mul Q mix :=
  (@RingQuotType_pack _ _ _ _ _ u mul Q _ _ _ _ id id mix).
Notation "[ 'ringQuotType' u & m 'of' Q ]" :=
  (@RingQuotType_clone _ _ _ _ _ u m Q _ _ id)
  (at level 0, format "[ 'ringQuotType'  u  &  m  'of'  Q ]") : form_scope.
Notation RingQuotMixin Q m1 mM :=
  (@RingQuotMixin_pack _ _ _ _ _ _ _ Q _ _ id _ _ id (zmod_quot_mixinP _) m1 mM).

Section PiRMorphism.

Monomorphic Variables (R : ringType) (equivR : rel R) (zeroR : R).

Monomorphic Variable Q : @ringQuotType R equivR zeroR -%R +%R 1 *%R.

Monomorphic Lemma pi_is_multiplicative : multiplicative \pi_Q.
Proof. by split; do ?move=> x y /=; rewrite !piE. Qed.

Monomorphic Canonical pi_rmorphism := AddRMorphism pi_is_multiplicative.
...
Monomorphic Variables (zeroT : T) (oppT : T -> T) (addT : T -> T -> T).
Monomorphic Variables (oneT : T) (mulT : T -> T -> T).
Monomorphic Variables (unitT : pred T) (invT : T -> T).

Monomorphic Record unit_ring_quot_mixin_of (Q : Type) (qc : quot_class_of T Q)
...
Monomorphic Record unit_ring_quot_class_of (Q : Type) : Type := UnitRingQuotClass {
...
Monomorphic Structure unitRingQuotType : Type := UnitRingQuotTypePack {
...
Monomorphic Definition unit_ring_quot_class rqT : unit_ring_quot_class_of rqT :=
  let: UnitRingQuotTypePack _ cT as qT' := rqT
    return unit_ring_quot_class_of qT' in cT.

Monomorphic Definition unit_ring_ring_quot_class rqT (rqc : unit_ring_quot_class_of rqT) :
  ring_quot_class_of eqT zeroT oppT addT oneT mulT rqT := RingQuotClass rqc.
Monomorphic Definition unit_ring_zmod_quot_class rqT (rqc : unit_ring_quot_class_of rqT) :
  zmod_quot_class_of eqT zeroT oppT addT rqT := ZmodQuotClass rqc.
Monomorphic Definition unit_ring_eq_quot_class rqT (rqc : unit_ring_quot_class_of rqT) :
  eq_quot_class_of eqT rqT := EqQuotClass rqc.

Monomorphic Canonical unitRingQuotType_eqType rqT :=
  Equality.Pack (unit_ring_quot_class rqT).
Monomorphic Canonical unitRingQuotType_choiceType rqT :=
  Choice.Pack (unit_ring_quot_class rqT).
Monomorphic Canonical unitRingQuotType_zmodType rqT :=
  GRing.Zmodule.Pack (unit_ring_quot_class rqT).
Monomorphic Canonical unitRingQuotType_ringType rqT :=
  GRing.Ring.Pack (unit_ring_quot_class rqT).
Monomorphic Canonical unitRingQuotType_unitRingType rqT :=
  GRing.UnitRing.Pack (unit_ring_quot_class rqT).
Monomorphic Canonical unitRingQuotType_quotType rqT :=
  QuotTypePack (unit_ring_quot_class rqT).
Monomorphic Canonical unitRingQuotType_eqQuotType rqT :=
  EqQuotTypePack (unit_ring_eq_quot_class (unit_ring_quot_class rqT)).
Monomorphic Canonical unitRingQuotType_zmodQuotType rqT :=
  ZmodQuotTypePack (unit_ring_zmod_quot_class (unit_ring_quot_class rqT)).
Monomorphic Canonical unitRingQuotType_ringQuotType rqT :=
...
Monomorphic Definition UnitRingQuotType_pack Q :=
...
Monomorphic Definition UnitRingQuotMixin_pack Q :=
...
Monomorphic Definition UnitRingQuotType_clone (Q : Type) qT cT
  of phant_id (unit_ring_quot_class qT) cT := @UnitRingQuotTypePack Q cT.

Monomorphic Lemma unit_ring_quot_mixinP rqT :
  unit_ring_quot_mixin_of (unit_ring_quot_class rqT) (unit_ring_quot_class rqT).
Proof. by case: rqT => [] ? [] ? ? []. Qed.

Monomorphic Lemma pi_unitr rqT : {mono \pi_rqT : x / unitT x >-> x \in GRing.unit}.
Proof. by case: rqT => [] ? [] ? ? []. Qed.

Monomorphic Lemma pi_invr rqT : {morph \pi_rqT : x / invT x >-> x^-1}.
Proof. by case: rqT => [] ? [] ? ? []. Qed.

Monomorphic Canonical pi_unit_quot_morph rqT := PiMono1 (pi_unitr rqT).
Monomorphic Canonical pi_inv_quot_morph rqT := PiMorph1 (pi_invr rqT).
...
Monomorphic Structure idealr (R : ringType) (S : {pred R}) := MkIdeal {
  idealr_zmod :> zmodPred S;
  _ : proper_ideal S
}.

Monomorphic Structure prime_idealr (R : ringType) (S : {pred R}) := MkPrimeIdeal {
...
Section ZmodQuotient2.

Monomorphic Variables (R : zmodType) (I : {pred R})
                      (zmodI : zmodPred I) (kI : keyed_pred zmodI).

Monomorphic Canonical equiv_equiv := EquivRelPack (@equiv_is_equiv R I zmodI kI).
Monomorphic Canonical equiv_encModRel := defaultEncModRel (@equiv R I zmodI kI).

Monomorphic Definition type := {eq_quot @equiv R I zmodI kI}.
Monomorphic Definition type_of of phant R := type.

Monomorphic Canonical rquot_quotType := [quotType of type].
Monomorphic Canonical rquot_eqType := [eqType of type].
Monomorphic Canonical rquot_choiceType := [choiceType of type].
Monomorphic Canonical rquot_eqQuotType := [eqQuotType @equiv R I zmodI kI of type].

End ZmodQuotient2.

Section ZmodQuotient3.

Variables (R : zmodType) (I : {pred R})
          (zmodI : zmodPred I) (kI : keyed_pred zmodI).

Lemma idealrBE x y : (x - y) \in kI = (x == y %[mod type kI]).
Proof. by rewrite piE equivE. Qed.

Lemma idealrDE x y : (x + y) \in kI = (x == - y %[mod type kI]).
Proof. by rewrite -idealrBE opprK. Qed.

Definition zero : type kI := lift_cst (type kI) 0.
Definition add := lift_op2 (type kI) +%R.
Definition opp := lift_op1 (type kI) -%R.
...
Section ZmodQuotient4.

Monomorphic Variables (R : zmodType) (I : {pred R})
                      (zmodI : zmodPred I) (kI : keyed_pred zmodI).

Monomorphic Canonical pi_zero_morph := PiConst (zero kI).
Monomorphic Canonical pi_opp_morph := PiMorph1 (pi_opp kI).
Monomorphic Canonical pi_add_morph := PiMorph2 (pi_add kI).

End ZmodQuotient4.

Section ZmodQuotient5.

Variables (R : zmodType) (I : {pred R})
          (zmodI : zmodPred I) (kI : keyed_pred zmodI).

Lemma addqA: associative (@add R I zmodI kI).
Proof. by move=> x y z; rewrite -[x]reprK -[y]reprK -[z]reprK !piE addrA. Qed.

Lemma addqC: commutative (@add R I zmodI kI).
Proof. by move=> x y; rewrite -[x]reprK -[y]reprK !piE addrC. Qed.

Lemma add0q: left_id (zero kI) (@add R I zmodI kI).
Proof. by move=> x; rewrite -[x]reprK !piE add0r. Qed.

Lemma addNq: left_inverse (zero kI) (@opp R I zmodI kI) (@add R I zmodI kI).
Proof. by move=> x; rewrite -[x]reprK !piE addNr. Qed.

End ZmodQuotient5.

Section ZmodQuotient6.

Monomorphic Variables (R : zmodType) (I : {pred R})
                      (zmodI : zmodPred I) (kI : keyed_pred zmodI).

Monomorphic Definition rquot_zmodMixin :=
  ZmodMixin (@addqA R I zmodI kI) (@addqC R I zmodI kI) (@add0q R I zmodI kI) (@addNq R I zmodI kI).
Monomorphic Canonical rquot_zmodType :=
  Eval hnf in ZmodType (@type R I zmodI kI) rquot_zmodMixin.

Monomorphic Definition rquot_zmodQuotMixin :=
  ZmodQuotMixin (@type R I zmodI kI) (lock _) (@pi_opp R I zmodI kI) (@pi_add R I zmodI kI).
Monomorphic Canonical rquot_zmodQuotType :=
  ZmodQuotType 0 -%R +%R (@type R I zmodI kI) rquot_zmodQuotMixin.

End ZmodQuotient6.

Notation "{quot I }" := (@type_of _ _ _ I (Phant _)).

Section RingQuotient1.
...
Monomorphic Canonical pi_one_morph (R : comRingType) (I : {pred R})
  (idealI : idealr I) (kI : keyed_pred idealI)
  := PiConst (@one R I idealI kI).

Section RingQuotient2.

Variables (R : comRingType) (I : {pred R})
          (idealI : idealr I) (kI : keyed_pred idealI).

Lemma pi_mul: {morph \pi : x y / x * y >-> @mul R I idealI kI x y}.
Proof.
move=> x y; unlock mul; apply/eqP; rewrite piE equivE.
rewrite -[_ * _](addrNK (x * repr (\pi_(@type R I idealI kI) y))) -mulrBr.
...
Monomorphic Canonical pi_mul_morph (R : comRingType) (I : {pred R})
  (idealI : idealr I) (kI : keyed_pred idealI)
  := PiMorph2 (@pi_mul R I idealI kI).

Section RingQuotient3.

Variables (R : comRingType) (I : {pred R})
          (idealI : idealr I) (kI : keyed_pred idealI).
Notation one := (@one R I idealI kI).
Notation mul := (@mul R I idealI kI).
...
Section RingQuotient4.

Monomorphic Variables (R : comRingType) (I : {pred R})
                      (idealI : idealr I) (kI : keyed_pred idealI).

Monomorphic Definition rquot_comRingMixin :=
  ComRingMixin (@mulqA R I idealI kI) (@mulqC R I idealI kI)
    (@mul1q R I idealI kI) (@mulq_addl R I idealI kI) (@nonzero1q R I idealI kI).

Monomorphic Canonical rquot_ringType    :=
  Eval hnf in RingType (@type R I idealI kI) rquot_comRingMixin.
Monomorphic Canonical rquot_comRingType :=
  Eval hnf in ComRingType (@type R I idealI kI) (@mulqC R I idealI kI).

Monomorphic Definition rquot_ringQuotMixin :=
  RingQuotMixin (@type R I idealI kI) (lock _) (@pi_mul R I idealI kI).
Monomorphic Canonical rquot_ringQuotType :=
  RingQuotType 1 *%R (@type R I idealI kI) rquot_ringQuotMixin.
